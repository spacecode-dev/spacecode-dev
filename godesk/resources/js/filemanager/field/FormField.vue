<template>
    <default-field :field="field">
        <template slot="field">
            <template v-if="field.value && field.display === 'image'">
                <div class="card relative card relative border-lg border-50 overflow-hidden px-0 py-0 max-w-xs mb-2">
                    <template v-if="field.type === 'image'">
                        <ImageDetail class="block w-full" :file="field" :css="''"/>
                    </template>
                    <template v-else>
            ...
            <UploadProgress ref="uploader" :current="currentPath" :visibility="field.visibility" :rules="field.upload_rules" v-on:removeFile="removeFileFromUpload"/>
            <file-select :id="field.name" :field="field" :is-readonly="field.readonly" :css="errorClasses" v-model="value" v-on:open-modal="openFilemanagerModal"/>
