<template>
    <div ref="filemanager-container">
        <heading class="mb-6">{{ this.__('Filemanager') }}</heading>
        ...
        <div class="card relative" id="filemanager-manager">
                ...
                <div class="w-full flex flex-wrap">
                    <div class="w-2/3 flex flex-wrap justify-start">
                        ...
                        <button v-if="multiSelecting && selectedFiles.length > 0" type="button" class="btn btn-default btn-small btn-danger text-white mr-3" @click="openMultiDeleteModal">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" aria-labelledby="delete" role="presentation">
                        ...
                        <div class="relative w-1/2 max-w-xs pr-3">
                            <template v-if="showFilters">
                                <select class="pl-search form-control form-input form-input-bordered w-full" v-model="filterBy" @change="filterFiles">
                                    <option value="">{{ this.__('Filter by...') }}</option>
            ...
            />
            <UploadProgress ref="uploader" :current="currentPath" v-on:removeFile="removeFileFromUpload"/>
        </div>
    </div>
</template>
...
export default {
    ...
    async created() {
        ...
        if (localStorage.getItem('godesk-filemanager-view')) {
            let viewS = localStorage.getItem('godesk-filemanager-view');
            if (['grid', 'list'].includes(viewS)) {
                this.view = viewS;
            } else {
                localStorage.setItem('godesk-filemanager-view', 'grid');
            }
        }
    ...
    methods: {
        ...
        viewAs(type) {
            this.view = type;
            localStorage.setItem('godesk-filemanager-view', type);
        },
