<template>
  <div ref="fileManagerContainer" id="filemanager-manager-container" class="p-3" :class="cssFilemanagerContainer" v-cloak>
    <nav class="bg-grey-light rounded font-sans w-full m-4">
      <ol class="list-reset flex text-grey-dark">
        <li><span class="text-blue font-bold cursor-pointer" @click="goToFolderNav(home)">{{ this.__('Home') }}</span></li>
      ...
      <div v-else class="px-2 overflow-y-auto files">
        <div class="flex flex-wrap -mx-2">
          <template v-if="files.error">
            <div class="w-full text-lg text-center my-4">
              {{ this.__('You don\'t have permissions to view this folder') }}
            </div>
          </template>
          ...
          <template v-else-if="!files.length">
            <div class="w-full text-lg text-center my-4">
              {{ this.__(`No ${filter || 'files or folders'} in current directory`, {filter: filter}) }}<br><br>
              <button v-if="buttons.delete_folder && !filter" class="btn btn-default btn-danger" @click="removeDirectory">
                {{ this.__('Remove directory') }}
              </button>
            </div>
          </template>
              ...
              <template v-if="!files.error">
                <template v-if="parent.id">
                  <div :class="filemanagerClass" :key="parent.id">
                    <Folder v-drag-and-drop:folder :ref="'folder_' + parent.id" :file="parent" :data-key="parent.id" class="h-40 folder-item" :class="{'loading': loadingInfo}"
