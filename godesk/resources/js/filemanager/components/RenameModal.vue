        ...
        <modal v-if="active" @modal-close="handleClose">
            <div class="bg-white rounded-lg shadow-lg " style="width: 600px;">
                <div class="p-8">
                    ...
                    <template v-else>
                        ...
                        <div class="flex flex-wrap items-stretch w-full mb-4 relative">
                            <input type="text" class="flex-shrink flex-grow flex-auto h-full form-control form-input form-input-bordered-left py-3" :placeholder="nameWithoutExtension" v-model="nameWithoutExtension"
                                   autofocus required v-on:keyup.enter="renamePath"/>
                ...
                <div class="bg-30 px-6 py-3 flex">
                    <div class="ml-auto">
                        <button type="button" data-testid="cancel-button" @click.prevent="cancelRename" class="btn text-80 font-normal h-9 px-3 mr-3 btn-link">{{ this.__('Cancel') }}</button>
                        <button ref="confirmButton" data-testid="confirm-button" :disabled="isSaving" @click.prevent="renamePath" class="btn btn-default btn-primary"
                                :class="{ 'cursor-not-allowed': isSaving, 'opacity-50': isSaving }">
                            <span v-if="isSaving">{{ this.__('Renaming') }}</span>
                            <span v-else>{{ this.__('Rename') }}</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            return api.rename(this.path, nameToSave).then(result => {
                ...
                if (result.success === true) {
                    ...
                    if (result.error) {
                        ...
                        this.$toasted.show(this.errorMsg, {type: 'error'});
                    }
                }
            });
        },
