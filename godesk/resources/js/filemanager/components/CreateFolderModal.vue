    ...
    <portal to="portal-filemanager" name="Remove File" transition="fade-transition">
        <modal v-if="active" @modal-close="handleClose" class="z-100">
            <div class="bg-white rounded-lg shadow-lg " style="width: 600px;">
                <div class="p-8">
                    <heading :level="2" class="mb-6">{{ this.__('Create folder') }}</heading>
                    <input type="text" class="w-full h-full form-control form-input form-input-bordered py-3" :placeholder="this.__('Write a folder name')" v-model="folderName" autofocus required
                ...
                <div class="bg-30 px-6 py-3 flex">
                    <div class="ml-auto">
                        <button type="button" data-testid="cancel-button" @click.prevent="cancelCreate" class="btn text-80 font-normal h-9 px-3 mr-3 btn-link">{{ this.__('Cancel') }}</button>
                        <button ref="confirmButton" data-testid="confirm-button" :disabled="isSaving" @click.prevent="createFolder" class="btn btn-default btn-primary"
                                :class="{ 'cursor-not-allowed': isSaving, 'opacity-50': isSaving }">
                            <span v-if="isSaving">{{ this.__('Creating') }}</span>
                            <span v-else>{{ this.__('Create') }}</span>
                        </button>
                    </div>
                </div>
            </div>
