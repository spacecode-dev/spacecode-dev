            ...
            <div class="bg-white rounded-lg shadow-lg" style="width: 70vw;">
                <div class="bg-30 flex flex-wrap border-b border-70">
                    <div class="w-3/4 px-4 py-3 ">
                        {{ this.__('Preview of') }} <span class="text-primary-70%">{{ info.name }}</span>
                    </div>
                    ...
                    <div class="w-2/5 bg-30 box-info flex flex-wrap">
                        <div class="info-data w-full">
                            <div class="info mx-4 my-3 flex flex-wrap items-center">
                                <span class="title bg-50 px-1 py-1 rounded-l">{{ this.__('Name') }}:</span>
                                ...
                                <template v-if="buttons.rename_file">
                                    ...
                                    <template v-if="editingName">
                                        ...
                                        <svg @click="rename" xmlns="http://www.w3.org/2000/svg" class="ml-1 cursor-pointer text-success fill-current" viewBox="0 0 20 20" width="12" height="12">
                                            <path d="M0 11l2-2 5 5L18 3l2 2L7 18z"/>
                                        </svg>
                                        <svg @click="editingName = !editingName" xmlns="http://www.w3.org/2000/svg" class="ml-1 cursor-pointer" viewBox="0 0 20 20" width="12" height="12">
                                            <path
                            ...
                            <div class="info mx-4 mt-6" v-if="info.url">
                                ...
                                <div class="flex flex-wrap items-stretch w-full mb-4 relative">
                                    ...
                                    <div class="flex -mr-px">
                                        <button class="copy flex items-center leading-normal bg-50 rounded rounded-l-none border border-l-0 border-70 px-3 whitespace-no-wrap text-grey-dark text-xs" v-copy="info.url"
                                                v-copy:callback="onCopy">{{ this.__('Copy') }}
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div class="info-actions mx-4 w-full justify-end">
                                <div class="">
                                    <a
                                        ...
                                        :href="`/nova-vendor/filemanager/actions/download-file?file=${this.info.path}`"
                                        target="_blank"
                                    >
                                        {{ this.__('Download') }}
                                    </a>
                        ...
                        <div class="info-actions w-full flex flex-wrap self-end justify-end">
                            <!-- <button type="button" data-testid="cancel-button" @click.prevent="removeFilePopup" class="btn text-danger text-sm font-normal h-9 px-3 mr-3 btn-link">{{ 'Remove file' }}</button> -->
                            <div :class="{ 'm-3': popup }">
                                <confirmation-button
                                    v-if="buttons.delete_file"
                                    :messages="messagesRemove"
                                    :css="'btn text-danger text-sm font-normal h-9 px-3 mr-3 btn-link'"
                                    v-on:confirmation-success="removeFilePopup()"/>
                                <template v-if="popup">
