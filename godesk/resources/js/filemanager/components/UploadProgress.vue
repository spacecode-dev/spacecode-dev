            ...
            <transition name="fade">
                <div class="flex flex-wrap w-full items-center">
                    <div class="preview w-1/3 text-70">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-12 h-12 flex justify-center items-center fill-current">
                    ...
                    <div class="w-2/3 text-xs">
                        <template v-if="error">
                            <div class="text-danger">{{ this.__('Error on upload') }}</div>
                        </template>
                        <template v-else>
                            {{ this.__('Uploading Folder') }} <small v-if="totalPercent === 100" class="text-success uppercase">{{ this.__('Success') }}</small>
            ...
            <transition name="fade">
                <div class="flex flex-wrap w-full items-center" v-bind:key="indexFiles" v-if="file.upload === true">
                    ...
                    <div class="w-2/3 text-xs">
                        ...
                        <template v-else>
                            {{ file.name | truncate(15) }} <small v-if="file.progress === 100" class="text-success uppercase">{{ this.__('Success') }}</small>
                            <progress-module :file="file"/>
                        </template>
                    </div>
                </div>
            </transition>
...
export default {
    ...
    methods: {
        ...
        uploadFileToServer(file, data, config) {
            window.axios
                .post('/nova-vendor/filemanager/uploads/add', data, config)
                .then(response => {
