            ...
            >
                ...
                <div class="missing p-8" v-if="missing">
                    <p class="text-center leading-normal">
                        <a :href="file.name" class="text-primary dim" target="_blank">{{ this.__('This image') }}</a> {{ this.__('could not be found.') }}
                    </p>
                </div>
...
export default {
    ...
    computed: {
        ...
        shouldShowHover() {
            return this.deletePermission || this.renamePermission || this.multiSelecting;
    ...
    methods: {
        ...
        getClassContainer() {
            if (this.view === 'list') {
                return 'block w-full h-full bg-center bg-contain no-repeat h-2/3';
            }
            return 'block w-full h-full bg-center bg-contain no-repeat h-2/3';
        },
        deleteFile(e) {
