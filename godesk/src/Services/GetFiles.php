...
trait GetFiles
{
    ...
    public function getFiles($folder, $order, $filter = false)
    {
        ...
        $cacheTime = config('godesk.filemanager.cache', false);
        ...
        return $this->orderData($files, $order, config('godesk.filemanager.direction', 'asc'));
    }
     ...
     * @return string
     */
    public function generateId(array $file): string
    {
        if (isset($file['timestamp'])) {
    ...
    {
        if (in_array(config('godesk.filemanager.disk'), $this->cloudDisks)) {
            return '';
        }
        return '/storage';
    }
     ...
     * @return array|false
     */
    public function getImageDimensionsFromCloud($file)
    {
        try {
            $client = new Client();
     ...
     * @return array
     */
    public function generateParent($folder): array
    {
     ...
     * @return Collection
     */
    public function getPaths($currentFolder): Collection
    {
